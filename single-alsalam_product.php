  ...
  <section class="relative bg-[#041424] min-h-[400px] flex flex-col justify-end pb-16 overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center pt-36">
      ...
      <nav class="flex items-center justify-center gap-2 mb-4 text-xs font-semibold text-white/50 tracking-wider uppercase font-sans">
        ...
        <a href="<?php echo esc_url(get_post_type_archive_link('alsalam_product') ?: home_url('/products')); ?>" class="hover:text-primary-light transition-colors duration-200"><?php echo esc_html(pll__('Products')); ?></a>
        <span class="text-white/30 font-light">/</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
        ...
        <div class="lg:col-span-5 space-y-6">
          ...
          <div class="bg-[#F4F7FE] rounded-[30px] p-6 border border-slate-100 flex items-center gap-4 text-start">
            ...
            <div>
              <h4 class="font-bold font-heading text-[#071D2C] text-sm"><?php echo esc_html(pll__('European Standards Approved')); ?></h4>
              <p class="text-xs text-slate-500 mt-0.5"><?php echo esc_html(pll__('Tested & validation-indexed for therapeutic hospital networks.')); ?></p>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-7 space-y-8 text-start">
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100 text-[#071D2C] font-bold text-xs uppercase tracking-wider">
                  <th class="px-6 py-4 text-start font-heading"><?php echo esc_html(pll__('Specification Parameter')); ?></th>
                  <th class="px-6 py-4 text-start font-heading"><?php echo esc_html(pll__('Metric Details')); ?></th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100 text-slate-600">
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__('Packaging presentation')); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html(pll__($tag1)); ?></td>
                </tr>
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__('Volume Availability')); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html(pll__($tag2)); ?></td>
                </tr>
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__('Quality Grade')); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html(pll__($tag3)); ?></td>
                </tr>
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__('Shelf life')); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html(pll__('36 Months from packaging date')); ?></td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="pt-4">
            ...
            >
              ...
              <span><?php echo esc_html(pll__('Request Commercial Inquiry')); ?></span>
            </a>
          </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <header class="flex flex-col items-center text-center mb-16">
        <h2 class="text-[#071D2C] text-2xl sm:text-3xl font-extrabold tracking-tight font-heading">
          <?php echo wp_kses_post(pll__('Related <span class="text-teal-500">Parenteral</span> Formulations')); ?>
        </h2>
