  ...
  <section class="relative bg-[#041424] min-h-[400px] flex flex-col justify-end pb-16 overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center pt-36">
      ...
      <nav class="flex items-center justify-center gap-2 mb-4 text-xs font-semibold text-white/50 tracking-wider uppercase font-sans">
        ...
        <a href="<?php echo esc_url(get_post_type_archive_link('alsalam_product') ?: home_url('/products')); ?>" class="hover:text-primary-light transition-colors duration-200"><?php echo esc_html(pll__(get_theme_mod('_alsalam_breadcrumb_products', 'Products'))); ?></a>
        <span class="text-white/30 font-light">/</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
        ...
        <div class="lg:col-span-5 space-y-6">
          ...
          <div class="bg-[#F4F7FE] rounded-[30px] p-6 border border-slate-100 flex items-center gap-4 text-start">
            ...
            <div>
              <h4 class="font-bold font-heading text-[#071D2C] text-sm"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_eu_title', 'European Standards Approved'))); ?></h4>
              <p class="text-xs text-slate-500 mt-0.5"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_eu_desc', 'Tested & validation-indexed for therapeutic hospital networks.'))); ?></p>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-7 space-y-8 text-start">
          <div>
            <span class="inline-block rounded-full bg-[#E5F0F6] px-4 py-1.5 text-xs font-semibold tracking-wide text-slate-800 font-sans mb-4">
              <?php echo esc_html(pll__(get_theme_mod('_alsalam_product_clinical_tab', 'Clinical Overview'))); ?>
            </span>
            <h2 class="text-2xl sm:text-3xl font-extrabold text-[#071D2C] tracking-tight mb-4 font-heading leading-tight">
              <?php echo esc_html(pll__(get_theme_mod('_alsalam_product_formulation_tab', 'Product Formulation & Intent'))); ?>
            </h2>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100 text-[#071D2C] font-bold text-xs uppercase tracking-wider">
                  <th class="px-6 py-4 text-start font-heading"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_spec_param', 'Specification Parameter'))); ?></th>
                  <th class="px-6 py-4 text-start font-heading"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_metric_details', 'Metric Details'))); ?></th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100 text-slate-600">
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_packaging_label', 'Packaging presentation'))); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html($tag1); ?></td>
                </tr>
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_volume_label', 'Volume Availability'))); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html($tag2); ?></td>
                </tr>
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_grade_label', 'Quality Grade'))); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html($tag3); ?></td>
                </tr>
                <tr>
                  <td class="px-6 py-4 font-medium text-[#071D2C]"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_shelf_label', 'Shelf life'))); ?></td>
                  <td class="px-6 py-4"><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_shelf_val', '36 Months from packaging date'))); ?></td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="pt-4">
            ...
            >
              ...
              <span><?php echo esc_html(pll__(get_theme_mod('_alsalam_product_inquiry_btn', 'Request Commercial Inquiry'))); ?></span>
            </a>
          </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <header class="flex flex-col items-center text-center mb-16">
        <h2 class="text-[#071D2C] text-2xl sm:text-3xl font-extrabold tracking-tight font-heading">
          <?php echo wp_kses_post(pll__(get_theme_mod('_alsalam_product_related_title', 'Related <span class="text-teal-500">Parenteral</span> Formulations'))); ?>
        </h2>
